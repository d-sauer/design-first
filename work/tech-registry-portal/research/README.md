# Research

This folder contains initial research and draft documents to help gather requirements and architectural considerations for the Tech Registry Portal.

## Documents

- [Product Requirements Draft](./product-requirements-draft.md) - Initial questions and ideas about product requirements
- [Architecture Draft](./architecture-draft.md) - Initial questions and ideas about system architecture
- [Schema](./schema/) - contains datamodel for managing technologies and what options are allowed

## Next Steps

Once you've completed the research drafts, proceed to:
1. Create the full Product Requirements Document - say "I want to define product definition"
2. Create the Architecture Documentation
